      ...
      if (serviceToken) {
        try {
          ...
          if (timeLeft <= 0) {
            ...
            const checkInterval = Math.max(timeLeft - 5 * 60 * 1000, 1000); // At least 1 second
            timer = setTimeout(checkSessionExpiration, checkInterval);
            console.log(`Next session check in ${checkInterval / 1000} seconds`);
          }
        } catch (err) {
